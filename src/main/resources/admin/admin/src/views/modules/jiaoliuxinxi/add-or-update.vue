	...
	<div class="addEdit-block" :style='{"padding":"0"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 16px 0","background":"none","display":"inline-block"}' class="date" v-if="type!='info'" label="交流日期" prop="jiaoliuriqi">
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
						v-model="ruleForm.jiaoliuriqi" 
						type="date"
						:readonly="ro.jiaoliuriqi"
						placeholder="交流日期"
					></el-date-picker> 
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 16px 0","background":"none","display":"inline-block"}' class="input" v-else-if="ruleForm.jiaoliuriqi" label="交流日期" prop="jiaoliuriqi">
					<el-input v-model="ruleForm.jiaoliuriqi" placeholder="交流日期" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 16px 0","background":"none","display":"inline-block"}' class="input" v-if="type!='info'"  label="发言人" prop="fayanren">
					<el-input v-model="ruleForm.fayanren" placeholder="发言人" clearable  :readonly="ro.fayanren"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 16px 0","background":"none","display":"inline-block"}' v-else class="input" label="发言人" prop="fayanren">
					<el-input v-model="ruleForm.fayanren" placeholder="发言人" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"width":"45%","margin":"0 2% 16px 0","background":"none","display":"inline-block"}' class="textarea" v-if="type!='info'" label="交流内容" prop="jiaoliuneirong">
					<el-input
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 16px 0","background":"none","display":"inline-block"}' v-else-if="ruleForm.jiaoliuneirong" label="交流内容" prop="jiaoliuneirong">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.jiaoliuneirong}}</span>
				</el-form-item>
...
export default {
	...
	created() {
		this.ruleForm.jiaoliuriqi = this.getCurDate()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='jiaoliuriqi'){
							this.ruleForm.jiaoliuriqi = obj[o];
							this.ro.jiaoliuriqi = true;
							continue;
						}
						if(o=='fayanren'){
							this.ruleForm.fayanren = obj[o];
							this.ro.fayanren = true;
							continue;
						}
				}
			...
			this.$http({
				...
				if (data && data.code === 0) {
					...
					if(((json.yonghuming!=''&&json.yonghuming) || json.yonghuming==0) && this.$storage.get("role")!="管理员"){
						this.ruleForm.fayanren = json.yonghuming
						this.ro.fayanren = true;
					}
					if(this.$storage.get("role")!="管理员") {
						this.ro.fayanren = true;
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiaoliuxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiaoliuxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
